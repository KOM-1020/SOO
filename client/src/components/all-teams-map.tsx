...
export default function AllTeamsMap({ day, type }: AllTeamsMapProps) {
  ...
  const initializeMap = (mapRef: React.RefObject<HTMLDivElement>, isReady: boolean) => {
    ...
    import('leaflet').then((L) => {
      ...
      mapInstanceRef.current = L.map(mapRef.current!).setView([42.0451, -87.6877], 10);
          ...
          scheduleData.forEach((customer: any, index: number) => {
            try {
              ...
              if (geocodeResult) {
                ...
                // Fallback to Chicago area with grid offset
                const fallbackPos = {
                  lat: 42.0451 + (index * 0.01),
                  lon: -87.6877 + (index * 0.01)
                };
      ...
      if (teamsWithData.length > 0) {
        mapInstanceRef.current.setView([42.0451, -87.6877], 10);
      }
