...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/load-csv-data", async (req, res) => {
    try {
      ...
      const optimizedSchedulePath = path.resolve(import.meta.dirname, "../attached_assets/travel_optimized_schedule_franchise_372_20250801_210849_1754206606722.csv");
      ...
      if (fs.existsSync(optimizedSchedulePath)) {
        ...
        const scheduleData = scheduleLines
          ...
          .map(line => {
            ...
            // Handle both old format and new format with date_shift
            let scheduleItem: ScheduleData;

            if (columns.length >= 10) {
              // New format with date_shift column
              const [
                dayName,
                teamNumber,
                startTime,
                ...
                city,
                customerId,
                cleanDate, // This column is present in the CSV but not used in the ScheduleData interface
                dateShift
              ] = columns;
              ...
              scheduleItem = {
                ...
                address,
                city,
                date_shift: parseInt(dateShift),
              };
            } else {
              // Old format without date_shift
              const [
                dayName,
                customerId,
                teamNumber,
  ...
  app.get("/api/dashboard-stats", async (req, res) => {
    try {
      ...
      // Load and parse original schedule data
      const originalPath = path.resolve(import.meta.dirname, "../attached_assets/Clean_FranchiseId_372_with_Address_Duration_1754221532976.csv");
      const optimizedPath = path.resolve(import.meta.dirname, "../attached_assets/travel_optimized_schedule_franchise_372_20250801_210849_1754206606722.csv");

      let originalData: any[] = [];
      let optimizedData: any[] = [];

      // Parse original data and calculate travel times
      if (fs.existsSync(originalPath)) {
        const csvData = fs.readFileSync(originalPath, 'utf-8');
        ...
        const rawData = lines
          ...
          .map(line => {
            const columns = line.split(',');
            const cleanDate = new Date(columns[3]?.trim());
            const timeIn = new Date(columns[7]?.trim());
            const timeOut = new Date(columns[8]?.trim());

            // Filter for July 7-13, 2025
